...
def precipitation():
    ...
    one_year_data =session.query(Measurement.date,Measurement.prcp).filter(Measurement.date >= one_year_most_recent).all()
    precipitation_dict = {}
    for date, prcp in one_year_most_recent:
...
def most_active():  
    ...
    most_active = session.query(Measurement.date, Measurement.tobs).filter(Measurement.station == 'USC00519281', Measurement.date >= one_year_most_recent)
    most_active_data= []
...
def temp_start (start):
    ...
    temperature_data = {    
        'Temp_Min': results[0].TMIN, 
        'Temp_Avg': results[0].TAVG,
        'Temp_Max': results[0].TMAX
    }
